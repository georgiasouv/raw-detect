...
#SBATCH --output=/networkhome/WMGDS/souval_g/raw-detect/cluster_scripts/logs/Syn02_%j.out
#SBATCH --error=/networkhome/WMGDS/souval_g/raw-detect/cluster_scripts/logs/Syn02_%j.err

# ============================================================
#  Syn02  =  freezing study, variant 'head'  (configs/methods/real_Syn02_head.py)
# ============================================================
...
# -- Training -----------------------------------------------------
echo "=== Starting Syn02 (variant_head) ==="
cd /networkhome/WMGDS/souval_g/raw-detect
export PYTHONPATH="$(pwd):${PYTHONPATH}"

mim train mmdet configs/methods/real_Syn02_head.py \
    --launcher none \
    --work-dir /networkhome/WMGDS/souval_g/raw-detect/work_dirs/Syn02_head \
    --cfg-options \
        visualizer.vis_backends.1.init_kwargs.name=Syn02_head \
...
wandb sync /networkhome/WMGDS/souval_g/raw-detect/work_dirs/Syn02_head/*/vis_data/wandb/offline-run-* 2>/dev/null

# -- Cleanup ------------------------------------------------------
kill $KRENEW_PID 2>/dev/null
echo "=== Syn02 (variant_head) finished ==="